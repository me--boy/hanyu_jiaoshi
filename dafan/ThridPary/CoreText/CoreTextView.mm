...
@interface CoreTextView ()<UIGestureRecognizerDelegate>
...
-(id)initWithFrame:(CGRect)frame
{
	if ((self=[super initWithFrame:frame])!=nil)
	{
        ...
        _tapGesture = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(doLink:)];
...
//				  [m_links setObject:attributes[@"href"] forKey:[NSValue valueWithCGRect:CGRectMake(bounds.origin.x, self.bounds.size.height-bounds.origin.y-m_contentInset.top-descent, bounds.size.width, bounds.size.height)]];
                  ...
                  [m_links setObject:attributes[@"href"] forKey:[NSValue valueWithCGRect:CGRectMake(bounds.origin.x, bounds.origin.y, bounds.size.width, bounds.size.height)]];
			  }
			  if (m_debugBorders)
			  {
...
- (BOOL) gestureRecognizer:(UIGestureRecognizer *)gestureRecognizer shouldReceiveTouch:(UITouch *)touch
{
    ...
    for (NSValue* rectValue in [m_links allKeys])
    {
        CGRect originRect = rectValue.CGRectValue;
        CGRect rect = CGRectMake(originRect.origin.x, 0, originRect.size.width, self.frame.size.height);
...
-(void)doLink:(UIGestureRecognizer*)gesture
{
	...
	{
        CGRect originRect = key.CGRectValue;
        CGRect rect = CGRectMake(originRect.origin.x, 0, originRect.size.width, self.frame.size.height);
        BOOL contain = CGRectContainsPoint(rect, point);
//        NSLog(@"%s, %@", __FUNCTION__, (contain ? @"contain" : @"not contain"));
		if (contain)
		{
//            NSLog(@"%s, %@", __FUNCTION__, @"do");
			if ([m_delegate respondsToSelector:@selector(coreTextView:openURL: rect:)])
            {
//                NSLog(@"%s, %@", __FUNCTION__, @"do 22");
                [m_delegate coreTextView:self openURL:url rect:rect];
            }
            else
            {
	...
	if ((self=[super init])!=nil)
	{
		...
		struct callbacks
		{
			static void startElement(HTMLParser* parser, const xmlChar* name,const xmlChar** atts)
			{
				...
				{
					[parser->m_style.lastObject setValue:(id)RGBCOLOR(0xea, 0x75, 0xa9).CGColor forKey:(id)kCTForegroundColorAttributeName];
					[parser->m_style.lastObject setValue:@(kCTUnderlineStyleSingle) forKey:(id)kCTUnderlineStyleAttributeName];
				}
...
//            [m_style setArray:@[@{@"font":@"Helvetica",@"size":@(18)}]];
//            [m_style addObject:@{@"" : @""}];
